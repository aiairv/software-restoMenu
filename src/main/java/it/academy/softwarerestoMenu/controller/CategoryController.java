...
public class CategoryController {
    ...
    public ResponseMessage<Category> restoreCategory(@PathVariable Long id) {
        try {
            return new ResponseMessage<>(
                    ...
                    ResultCode.SUCCESS,
                    "Категория успешно восстановлена!",
                    ResultCode.SUCCESS.getHttpCode());
        } catch (Exception e) {
    ...
    public ResponseMessage<Category> update(@RequestBody Category category) {
        if (category.getId() == null)
            throw new CategoryNotFoundException("CategoryController: update()  id is null");
        try {
            return new ResponseMessage<>(
                    service.save(category),
                    ResultCode.SUCCESS, "Категория успешно обновлена!",
                    ResultCode.SUCCESS.getHttpCode());
        } catch (Exception exception) {
    ...
    public ResponseMessage<Long> delete(@PathVariable Long id) {
        try {
            return new ResponseMessage<>(
                    service.delete(id),
                    ResultCode.SUCCESS, "Категория успешно удалена!",
                    ResultCode.SUCCESS.getHttpCode());
        } catch (Exception e) {
